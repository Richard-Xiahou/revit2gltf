...
namespace Revit2Gltf.glTF
{
    public class glTFUtil
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetLevelParameter(Element element) {
            string level = "无标高";
            foreach (Parameter p in element.Parameters)
            {
        ...
        /// </summary>
        /// <param name="c"></param>
        public static void RecursionCategory(List<string> categoryList, Category c)
        {
            if (c != null)
            {
                categoryList.Add(c.Name);
                RecursionCategory(categoryList, c.Parent);
            }
        }
        public static string GetCategoryLayerString(Category c) {
            var categoryList = new List<string>();

            glTFUtil.RecursionCategory(categoryList, c);
